...
func TestNewAsyncContext(t *testing.T) {
	...
	ac, err := argyll.NewAsyncContext(ctx)
...
func TestAsyncContextMissingMeta(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name:    "no metadata",
			meta:    nil,
			wantErr: argyll.ErrMetadataNotFound,
		},
		{
			name:    "missing webhook_url",
			meta:    api.Metadata{},
			wantErr: argyll.ErrWebhookURLNotFound,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := argyll.NewAsyncContext(ctx)
...
func TestAsyncContextComplete(t *testing.T) {
	...
	client := argyll.NewClient(server.URL, 30*time.Second)
	ctx := &argyll.StepContext{
		Context:  context.Background(),
		Client:   client.Flow("test-flow"),
		StepID:   "test-step",
...
func TestAsyncContextWebhookError(t *testing.T) {
	...
	assert.ErrorIs(t, err, argyll.ErrWebhookError)
