...
func TestNewClient(t *testing.T) {
	...
	client := argyll.NewClient("http://localhost:8080", timeout)
...
func TestFlowGetState(t *testing.T) {
	...
	fl, err := fc.GetState(context.Background())
	assert.NoError(t, err)
	assert.Equal(t, api.FlowID("my-flow"), fl.ID)
...
func TestGetStateNetworkError(t *testing.T) {
	client := argyll.NewClient("http://localhost:1", 1*time.Millisecond)
	fc := client.Flow("test-flow")
