...
func newRaftTestConfig(t *testing.T) *config.Config {
	...
	forwardAddr := availableAddress(t)
	port := availablePort(t)
	nodeID := "test-node-" + strconv.Itoa(port)

	cfg.APIPort = port
	cfg.Raft.LocalID = nodeID
	cfg.Raft.BindAddress = addr
	cfg.Raft.AdvertiseAddress = addr
	cfg.Raft.ForwardBindAddress = forwardAddr
	cfg.Raft.ForwardAdvertiseAddress = forwardAddr
