...
func TestFlowStateTimeoutScheduled(t *testing.T) {
	initialState := &api.FlowState{
		ID:     "test-flow",
		Status: api.FlowActive,
	}
	...
	eventData := api.TimeoutScheduledEvent{
		...
		UpstreamStepIDs: []api.StepID{"upstream1"},
	}
	data, err := json.Marshal(eventData)
	assert.NoError(t, err)
	...
	assert.NotNil(t, result)
	assert.NotNil(t, result.Timeouts["step1"])
	timeout := result.Timeouts["step1"]
	assert.True(t, timeout.FiresAt.Equal(firesAt))
	assert.Len(t, timeout.Attributes, 2)
	assert.Len(t, timeout.UpstreamStepIDs, 1)
	assert.True(t, result.LastUpdated.Equal(now))
}

func TestFlowStateTimeoutFired(t *testing.T) {
	initialState := &api.FlowState{
		ID:     "test-flow",
		Status: api.FlowActive,
		Timeouts: api.Timeouts{
			"step1": {
				FiresAt:         time.Now().Add(-1 * time.Second),
				Attributes:      []api.Name{"attr1"},
				UpstreamStepIDs: []api.StepID{"upstream1"},
			},
		},
	}
	...
	assert.NotNil(t, result)
	assert.Nil(t, result.Timeouts["step1"])
	assert.True(t, result.LastUpdated.Equal(now))
}

func TestTimeoutFiredWithMultipleTimeouts(t *testing.T) {
	now := time.Now()
	initialState := &api.FlowState{
		ID:     "test-flow",
		Status: api.FlowActive,
		Timeouts: api.Timeouts{
			"step1": {FiresAt: now.Add(-1 * time.Second)},
			"step2": {FiresAt: now.Add(10 * time.Second)},
			"step3": {FiresAt: now.Add(20 * time.Second)},
		},
	}
	...
	assert.NotNil(t, result.Timeouts["step1"])
	assert.Nil(t, result.Timeouts["step2"])
	assert.NotNil(t, result.Timeouts["step3"])
}

func TestFlowStateTimeoutCanceled(t *testing.T) {
	initialState := &api.FlowState{
		ID:     "test-flow",
		Status: api.FlowActive,
		Timeouts: api.Timeouts{
			"step1": {
				FiresAt:         time.Now().Add(10 * time.Second),
				Attributes:      []api.Name{"attr1"},
				UpstreamStepIDs: []api.StepID{"upstream1"},
			},
		},
...
func TestFlowStateTimeoutCanceledNotFound(t *testing.T) {
	initialState := &api.FlowState{
		ID:     "test-flow",
		Status: api.FlowActive,
		Timeouts: api.Timeouts{
			"step1": {FiresAt: time.Now().Add(10 * time.Second)},
		},
	}
	now := time.Now()
