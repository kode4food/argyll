...
const (
	...
	DefaultRaftBindAddress     = "127.0.0.1:9701"
...
func (c *Config) LoadFromEnv() error {
	...
	if raftBindAddress := os.Getenv("RAFT_BIND_ADDRESS"); raftBindAddress != "" {
		c.Raft.BindAddress = raftBindAddress
	}
	if raftAdvertiseAddress := os.Getenv("RAFT_ADVERTISE_ADDRESS"); raftAdvertiseAddress != "" {
		c.Raft.AdvertiseAddress = raftAdvertiseAddress
	}
	if forwardBindAddress := os.Getenv("RAFT_FORWARD_BIND_ADDRESS"); forwardBindAddress != "" {
		c.Raft.ForwardBindAddress = forwardBindAddress
	}
	if forwardAdvertiseAddress := os.Getenv("RAFT_FORWARD_ADVERTISE_ADDRESS"); forwardAdvertiseAddress != "" {
		c.Raft.ForwardAdvertiseAddress = forwardAdvertiseAddress
	}
...
func parseRaftServers(spec string) ([]raft.Server, error) {
	...
	for _, part := range parts {
		...
		id, addrSpec, ok := strings.Cut(part, "=")
		id = strings.TrimSpace(id)
		addrSpec = strings.TrimSpace(addrSpec)
		addr, forwardAddr, _ := strings.Cut(addrSpec, "|")
		addr = strings.TrimSpace(addr)
		forwardAddr = strings.TrimSpace(forwardAddr)
		if !ok || id == "" || addr == "" {
