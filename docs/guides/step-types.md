...
Any step type can process multiple items using `required.for_each` or `optional.for_each`. The engine creates one work item per array element and aggregates results.
...
```json
{
  ...
  "attributes": {
    "orders": { "role": "required", "type": "array", "required": { "for_each": true } },
...
```json
{
  ...
  "attributes": {
    "user": { "role": "required", "required": { "mapping": { "name": "username" } } },
    "authorized": { "role": "output", "output": { "mapping": { "name": "is_authorized" } } }
  }
}
```
